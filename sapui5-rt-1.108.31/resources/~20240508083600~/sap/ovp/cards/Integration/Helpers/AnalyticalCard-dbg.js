sap.ui.define([
    ...
    function setVizPropertyMap(sKey, oVizProperties) {
        if (oVizPropertyMap && !oVizPropertyMap[sKey]) {
            ...
            var savedVizProperties = oVizPropertyMap[sKey];
            for (var key in oVizProperties) {
                if (
                    oVizProperties.hasOwnProperty(key) &&
                    savedVizProperties.hasOwnProperty(key) &&
                    savedVizProperties[key] !== oVizProperties[key]
                ) {
                    savedVizProperties[key] = oVizProperties[key];
                }
            }
     ...
     * @private
     */
    function _getDimensionValueBinding(sDataType, sPropertyPath, oEntityType) {
        if (sDataType === "date") {
            var aPropertyInfo = oEntityType.property.filter(function (oProperty) {
    ...
    function createChartContent(oCardDefinition, sChartAnnotationPath, oSapCard) {
        ...
        var oChartAnnotation = oEntityType[sChartAnnotationPath],
            sTitle = (oChartAnnotation.Title && oChartAnnotation.Title.String) || "",
            sChartType = oVizFrame.getVizType(),
            sMeasureKey = sCardId,
            sDimensionkey = sCardId,
            sTitleKey,
            sTitleVal;
        if (!sTitle && oView.byId("ovpCT")) {
            sTitle = oView.byId("ovpCT").getText();
        }
        ...
        var dimensionFn = function (sDimKey, sCardId, oEntityType) {
            return function (oDim) {
                ...
                return {
                    ...
                    value: _getDimensionValueBinding(oDim.getProperty("dataType"), sPropertyPath, oEntityType),
        ...
        if (!oBatchObject.header) {
            oChartContent.data.path = "/d/results";
        }
