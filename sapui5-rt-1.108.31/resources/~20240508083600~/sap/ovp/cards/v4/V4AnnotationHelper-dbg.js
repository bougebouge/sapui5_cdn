    ...
    */
    ...
    function _formatField(
        ...
        if (!bIncludeOnlyUOM) {
            ...
            if (
                oEntityTypeProperty &&
                (oEntityTypeProperty.$Type === "Edm.DateTime" || oEntityTypeProperty.$Type === "Edm.DateTimeOffset")
            ) {
                ...
                result = generatePathForField(
                    [oItem.Value.$Path ? oItem.Value.$Path : sProperty.split("/")[sProperty.split("/").length - 1]],
                    functionName,
                    oStaticValues
                );
            } else if (
                (oItem.ValueFormat && oItem.ValueFormat.NumberOfFractionalDigits >= 0) ||
                ...
                var iScale =
                    oItem.ValueFormat && oItem.ValueFormat.NumberOfFractionalDigits
                        ? oItem.ValueFormat.NumberOfFractionalDigits
                        : 0,
                    sUnitPath;
                var iScaleFactor =
                ...
                if (oEntityTypeProperty["@" + ANNOTATIONTYPE.ISO_CURRENCY]) {
                    ...
                    sUnitPath = oEntityTypeProperty["@" + ANNOTATIONTYPE.ISO_CURRENCY].$Path
                        ...
                        : oEntityTypeProperty["@" + ANNOTATIONTYPE.ISO_CURRENCY];
                    ...
                    sUnitPath = oEntityTypeProperty["@" + ANNOTATIONTYPE.UOM].$Path
                        ...
                        : oEntityTypeProperty["@" + ANNOTATIONTYPE.UOM];
                ...
                var oUnitProperty = sUnitPath ? oMetaModel.getProperty("/" + oEntityType.$Type)[sUnitPath] : null,
                    aParts = [
                        oItem.Value.$Path
                            ? oItem.Value.$Path
                            : sProperty.split("/")[sProperty.split("/").length - 1]
                    ];
    ...
    function getSelectFields(iContext, oMetaModel, oEntityType, aAnnotationsPath) {
        ...
        for (var i = 0; i < aAnnotationsPath.length; i++) {
            ...
            var sEntitySetName = V4MetadataAnalyzer.getEntitySetName(oMetaModel, oEntityType.$Type);
            sAnnotationPath = sEntitySetName + "/@" + aAnnotationsPath[i];
    ...
    function getAggregateNumber(iContext, oEntitySet, oDataPoint, oSelectionVariant) {
        ...
        var sCustomParameter = bCheckFilterPreference
            ...
            : "";
        var parameters =
            "parameters:{'$$aggregation' : {'aggregate' : {" + paramsArr.join(",") + "}}" + sCustomParameter + "}";
    ...
    function getUnitColumnV4(measure, properties, forSubtitleUOM) {
        ...
        for (obj in properties) {
            ...
            if (
                properties[obj].annotations &&
                properties[obj].annotations.hasOwnProperty("@" + ANNOTATIONTYPE.ISO_CURRENCY)
            ) {
                //as part of supporting V4 annotation
                sPath = properties[obj].annotations["@" + ANNOTATIONTYPE.ISO_CURRENCY].$Path;
                sString = properties[obj].annotations["@" + ANNOTATIONTYPE.ISO_CURRENCY].$String;
                if (sPath) {
                    return sPath;
                } else {
            ...
            if (
                properties[obj].annotations &&
                properties[obj].annotations.hasOwnProperty("@" + ANNOTATIONTYPE.UOM)
            ) {
                sPath = properties[obj].annotations.hasOwnProperty("@" + ANNOTATIONTYPE.UOM).$Path;
                sString = properties[obj].annotations["@" + ANNOTATIONTYPE.UOM].$String;
                if (sPath) {
                    tempUnit = sPath;
                } else {
