sap.ui.define([
    ...
    return CardController.extend("sap.ovp.cards.quickview.Quickview", {
        ...
        onBeforeRendering: function () {
            ...
            var bShowFirstActionInFooter = oCardPropertiesModel.getProperty("/showFirstActionInFooter")
                ? oCardPropertiesModel.getProperty("/showFirstActionInFooter")
                : false;
                ...
                if (!!oFooter && !!aCustomActions && !!aCustomActions.length) {
                    for (var j = 0; j < aCustomActions.length; j++) {
                        ...
                        if (!!oMainComponent && !!text && !!aCustomActions[j].press) {
                            pressEvent = oMainComponent.templateBaseExtension.provideCustomActionPress(aCustomActions[j].press)
                                ? oMainComponent.templateBaseExtension.provideCustomActionPress(aCustomActions[j].press)
                                : oMainComponent.onCustomActionPress(aCustomActions[j].press);
