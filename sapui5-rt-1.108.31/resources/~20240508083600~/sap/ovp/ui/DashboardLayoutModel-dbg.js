sap.ui.define([
    ...
    LayoutModel.prototype.updateCardVisibility = function (aChgCards) {
        ...
        for (var i = 0; i < aChgCards.length; i++) {
            ...
            if (!aChgCards[i].visibility) {
                ...
                oCardLayoutVariant[0].dashboardLayout.row =
                    oCardVariant[0].dashboardLayout["C" + this.iColCount].row;
            }
                        ...
                        if (oLayoutCard.length > 0) {
                            ...
                            if (Array.isArray(oCard) && oCard.length > 0) {
                                ...
                                if (oCardLayoutObj) {
                                    ...
                                    oCardLayoutObj.colSpan =
                                        oCard[0].template === "sap.ovp.cards.stack"
                                            ? 1
                                            : Math.min(oVariant[item].colSpan, this.iColCount);
    ...
    LayoutModel.prototype._setCardSpanFromDefault = function (oCard) {
        ...
        if (!oCard.settings.defaultSpan) {
            ...
            if (oCard.settings.defaultSpan.showOnlyHeader) {
                oCard.dashboardLayout.rowSpan = Math.ceil(
                    (oCardProp.headerHeight + 2 * this.iCardBorderPx) / this.iRowHeightPx
                );
                ...
                if (oCard.template === "sap.ovp.cards.linklist" || oCard.template === "sap.ovp.cards.v4.linklist") {
                    oCard.dashboardLayout.rowSpan = oCard.settings.defaultSpan.rows
                        ? oCard.settings.defaultSpan.rows
                        : 1;
                    oCard.dashboardLayout.autoSpan = false;
                } else {
                ...
                oCard.dashboardLayout.noOfItems = oCard.settings.defaultSpan.rows
                    ? oCard.settings.defaultSpan.rows
                    : oCardProp.noOfItems;
                oCard.dashboardLayout.showOnlyHeader = false;
            }
            if (oCard.template === "sap.ovp.cards.stack") {
                ...
                oCard.dashboardLayout.colSpan = oCard.settings.defaultSpan.cols
                    ? Math.min(oCard.settings.defaultSpan.cols, this.iColCount)
                    : 1;
            }
            oCard.dashboardLayout.maxColSpan = oCard.settings.defaultSpan.cols
                ? oCard.settings.defaultSpan.cols
                : 1;
        }
    ...
    LayoutModel.prototype._sliceSequenceSausage = function (oUseVariant) {
        var i = 0,
            ...
            oCard = {},
            aSliceCols = [],
            sCurrentLayoutKey = "C" + this.iColCount,
            oPreviousLREPData;
        ...
        for (i = 0; i < this.aCards.length; i++) {
            ...
            oCard.dashboardLayout.colSpan =
                oCard.dashboardLayout.colSpan > this.iColCount ? this.iColCount : oCard.dashboardLayout.colSpan;
            ...
            if (Array.isArray(oLayoutCard) && oLayoutCard.length > 0 && !oLayoutCard[0].visibility) {
                ...
                for (
                    j = oCard.dashboardLayout.column;
                    j < oCard.dashboardLayout.column + oCard.dashboardLayout.colSpan;
                    j++
                ) {
    ...
    LayoutModel.prototype._sortCardsByRow = function (aCards) {
        ...
        aCards.sort(function (card1, card2) {
            ...
            if (
                card1.dashboardLayout.column &&
                card1.dashboardLayout.row &&
                card1.dashboardLayout.row === card2.dashboardLayout.row
            ) {
                ...
                if (affectedCards[i].id === this.aCards[j].id || !affectedCards[i].dashboardLayout.visible) {
                    ...
                    if (flag === true) {
                        ...
                        if (verticalDragFlag) {
                            ...
                            if (
                                newCardPosition.row < originalCardCopy.dashboardLayout.row &&
                                newCardPosition.row === this.aCards[j].dashboardLayout.row
                            ) {
                                affectedCards[i].dashboardLayout.row = this.aCards[j].dashboardLayout.row;
                                oldRow = this.aCards[j].dashboardLayout.row;
                                this.aCards[j].dashboardLayout.row =
                                    affectedCards[i].dashboardLayout.row + affectedCards[i].dashboardLayout.rowSpan;
                                if (aDragOrResizeChanges) {
                                    aDragOrResizeChanges.push({
                                        changeType: "dragOrResize",
                                        content: {
                                ...
                                newCardPosition.row >
                                originalCardCopy.dashboardLayout.row + this.aCards[j].dashboardLayout.rowSpan
                            ) {
                                oldRow = this.aCards[j].dashboardLayout.row;
                                this.aCards[j].dashboardLayout.row = originalCardCopy.dashboardLayout.row;
                                affectedCards[i].dashboardLayout.row =
                                    this.aCards[j].dashboardLayout.row + this.aCards[j].dashboardLayout.rowSpan;
    ...
    LayoutModel.prototype._checkOverlapOfCards = function (originalCard, affectedCard) {
        ...
        if (
            (affectedCardStartColumn >= originalCardStartColumn &&
                affectedCardStartColumn < originalCardEndColumn) ||
        ...
        if (
            (affectedCardStartRow >= originalCardStartRow && affectedCardStartRow < originalCardEndRow) ||
        ...
        for (var iCount = 1; iCount <= this.iColCount; iCount++) {
            ...
            if (!!aArray) {
                ...
                if (!!oObj) {
                    ...
                    var iRow = oObj.dashboardLayout.row === undefined ? 0 : +oObj.dashboardLayout.row;
                    var iRowSpan = oObj.dashboardLayout.rowSpan === undefined ? 0 : +oObj.dashboardLayout.rowSpan;
                    maxHeightArr.push(iRow + iRowSpan);
                }
            }
        }
        //Take the maximum height from the array which is equal to the height of the container
        iHighestRow = Math.max.apply(
            Math,
            maxHeightArr.map(function (ele) {
                return ele;
            })
        );
        return iHighestRow;
    ...
    LayoutModel.prototype._getDefaultCardItemHeightAndCount = function (oCardProperties) {
        ...
        if (oCardProperties) {
            ...
            var titleRow =
                oCardProperties.settings.defaultSpan && oCardProperties.settings.defaultSpan.minimumTitleRow
                    ? oCardProperties.settings.defaultSpan.minimumTitleRow
                    : 1;
            var subTitleRow =
                oCardProperties.settings.defaultSpan && oCardProperties.settings.defaultSpan.minimumSubTitleRow
                    ? oCardProperties.settings.defaultSpan.minimumSubTitleRow
                    : 1;
            var headerType =
                oCardProperties.settings.dataPointAnnotationPath ||
                    (oCardProperties.settings.tabs && oCardProperties.settings.tabs[0].dataPointAnnotationPath)
                    ? "KPIHeader"
                    : "NormalHeader";
