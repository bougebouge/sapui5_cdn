...
sap.ui.define([
    ...
    function AppLifeCycleAgent () {
        var that = this,
            ...
            fnCreateApplication,
            oCachedApplications = {},
            oCachedApplicationsComponentName = {},
                    ...
                    oServiceCalls: {
                        create: {
                            executeServiceCallFn: function (oMessageData) {
                                ...
                                return that.create(sAppIdNew, oMsg.body.sUrl);
                            }
                        },
                        destroy: {
                            executeServiceCallFn: function (oMessageData) {
        ...
        this.create = function (appId, sUrl) {
            ...
            that.getAppInfo(appId, sUrl).then(function (oAppInfo) {
                var sKeepAliveAppKey;
                if (Object.keys(oCachedApplications).length > 0) {
                    sKeepAliveAppKey = Object.keys(oCachedApplications).find(function (sCachedKey) {
                        var sKeepAliveComponentName = oCachedApplicationsComponentName[oCachedApplications[sCachedKey].sId];
                        var sCurrComponentName = (oAppInfo.ui5ComponentName || oAppInfo.name);
                        return sKeepAliveComponentName && sKeepAliveComponentName === sCurrComponentName;
                    });
                    if (sKeepAliveAppKey) {
                        that.destroy(sKeepAliveAppKey);
                    }
                }
                that.getURLParameters(new URI(sUrl).query(true)).then(function (oURLParameters) {
                    fnCreateApplication(appId, oURLParameters, oAppInfo).then(function (oCreateApplicationResult) {
        ...
        this.destroy = function (sStorageKey) {
            ...
            if (sStorageKey && oCachedApplications[sStorageKey]) {
                ...
                delete oCachedApplicationsComponentName[oCachedApplications[sStorageKey].sId];
                ...
                delete oCachedApplicationsComponentName[oRunningApp.sId];
        ...
        this.setComponent = function (oApp, ui5ComponentName) {
            oRunningApp = oApp;
            ...
            if (oRunningApp) {
                ...
                oCachedApplicationsComponentName[oRunningApp.sId] = ui5ComponentName;
            }
        };
