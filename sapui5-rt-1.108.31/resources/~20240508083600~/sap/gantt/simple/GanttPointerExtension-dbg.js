...
sap.ui.define([
	...
	var GanttPointerHelper = {
		addEventListeners: function(oGantt) {
			...
			this.bindBackgroundRowEvent($ganttSvg, oExtension, oGantt);
		...
		bindBackgroundRowEvent: function($ganttSvg, oExtension, oGantt) {
			function attachEventListener(oRow) {
				jQuery(oRow).on(BrowserEvent.mouseenter + sNamespace + " " + BrowserEvent.mouseleave + sNamespace, function(oEvent) {
					...
					if (oDragExtension._bEnableRowHighlight && oDragExtension.isDragging()) {
						oExtension._bIsMouseOnCorrectRow = false;
