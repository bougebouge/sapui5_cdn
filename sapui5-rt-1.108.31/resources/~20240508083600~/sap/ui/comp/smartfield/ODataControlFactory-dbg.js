...
sap.ui.define([
	...
	ODataControlFactory.prototype._handleEventingForTextArrangement = function(oSettings) {
		...
		oControl.attachChange(function (oControlEvent) {
			var oNewEvent = this._getChangeEventParams(oControlEvent),
				...
				bSuggestionsOpened = bHasSuggestions && oControl._oSuggPopover.isOpen();

			// when suggestions are enabled, suggestions popover is opened and "validated" parameter is missing we suppress for additional firing
			// as the change event was fired from base control
			if (!oNewEvent.validated && bHasSuggestions && bSuggestionsOpened) {
				return;
			}
	...
	ODataControlFactory.prototype.updateModelPropertiesForCurrency = function(sPropertyName, oSettings) {
		...
		if (oCurrencyField) {
			...
			if (this._getDisplayBehaviourConfiguration() || oCurrencySmartField && (oCurrencySmartField.getEditable &&
				(!oCurrencySmartField.getEditable() || (oCurrencySmartField._oFactory._getDisplayBehaviourConfiguration() || oCurrencySmartField._oFactory._getTextArrangementType())))) {
