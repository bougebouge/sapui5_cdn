...
sap.ui.define([
	...
	function getAdditionalSelects(oBinding) {
		...
		for (i = 0; i < aSelect.length; i += 1) {
			...
			if (oDimension) {
				var oDimensionDetails = oBinding.oDimensionDetailsSet[oDimension.getName()];
...
//				delete aKI entries n_e ... nPrime_e - 1
	...
	AnalyticalBinding.prototype._addSorters = function (oSortExpression, aGroupingSorters) {
		if (this._canApplySortersToGroups()) {
			this.aSorter.forEach(function (oApplicationSorter) {
				AnalyticalBinding._addSorter(oApplicationSorter, oSortExpression);
			});
			aGroupingSorters.forEach(function (oGroupingSorter) {
	...
	AnalyticalBinding.prototype._mergeAndAddSorters = function (aGroupingSorters, oSortExpression) {
		var aApplicationSorters = this.aSorter.slice(),
			that = this;

		aGroupingSorters.forEach(function (oGroupingSorter) {
			var sDimensionName = oGroupingSorter.sPath;
			for (var i = 0; i < aApplicationSorters.length; i += 1) {
				var oApplicationSorter = aApplicationSorters[i];
				var sPath = oApplicationSorter.sPath;
				if (sPath === sDimensionName || that.oDimensionDetailsSet[sDimensionName].textPropertyName === sPath) {
					AnalyticalBinding._addSorter(oApplicationSorter, oSortExpression);
		...
		aApplicationSorters.forEach(function (oApplicationSorter) {
