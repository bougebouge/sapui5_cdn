import { InnerControlType } from "sap/fe/templates/ListReport/controls/MultipleModeControl";
...
export class LRMessageStrip {
	...
	createCustomMessage(
		event: Event | null,
		oData: { message: LRCustomMessage | undefined; table: Table; skipMessageManagerUpdate?: boolean }
	) {
	...
	onSelectMultipleModeControl(event: Event, controller: ListReportController) {
		const table = controller._getTable() as Table;
	...
	showCustomMessage(
		...
		table = controller._getTable() as Table;
		this.createCustomMessage(null, { message, table });
	}
}
