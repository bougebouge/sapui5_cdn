...
class IntentBasedNavigation extends ControllerExtension {
	...
	adaptContextPreparationStrategy(oNavigationInfo: { semanticObject: string; action: string }) {
	...
	@extensible(OverrideExecution.After)
	// eslint-disable-next-line @typescript-eslint/no-unused-vars
	adaptNavigationContext(oSelectionVariant: SelectionVariant, oNavigationInfo: { semanticObject: string; action: string }) {
		// to be overriden by the application
	}
