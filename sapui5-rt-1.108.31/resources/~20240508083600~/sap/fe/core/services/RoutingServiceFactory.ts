...
export class RoutingService extends Service<RoutingServiceSettings> {
	...
	_parseRoutingConfiguration(oRoutingConfig: any, oRootViewConfig: any) {
		const isFCL =
			oRootViewConfig?.viewName === "sap.fe.core.rootView.Fcl" ||
			oRootViewConfig?.viewName === "sap.fe.templates.RootContainer.view.Fcl";
				...
				const aResolvedParameterPromises = aParts.map(function (oPathPart: any) {
					...
					const localContext =
						localContextPath === oContext.getPath()
							? oContext
							: (oContext.getModel().bindContext(localContextPath).getBoundContext() as Context);
					...
					return localContext.requestProperty(aRelativeParts[aRelativeParts.length - 1]).then(function (oValue) {
						const oPropertyInfo = oMetaContext.getObject() as any; // Returns any in master for some reason
	...
	_checkIfContextSupportsSemanticPath(oContext: Context) {
		...
		const oMetaModel = oContext.getModel().getMetaModel() as ODataMetaModel;
		const sEntitySetName = oMetaModel.getMetaContext(oContext.getPath()).getObject("@sapui.name") as any as string;
