...
import type { ManagedObjectEx, ServiceContext } from "types/extension_types";
...
class ResourceModelServiceFactory extends ServiceFactory<ResourceModelServiceSettings> {
	...
	createInstance(oServiceContext: ServiceContext<ResourceModelServiceSettings>) {
		const sKey =
			`${(oServiceContext.scopeObject as ManagedObjectEx).getId()}_${oServiceContext.settings.bundles.join(",")}` +
