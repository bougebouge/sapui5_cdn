...
import type { BaseAction, CombinedAction, CustomAction } from "sap/fe/core/converters/controls/Common/Action";
import { getActionsFromManifest, removeDuplicateActions } from "sap/fe/core/converters/controls/Common/Action";
import { TableControlConfiguration, TableVisualization } from "sap/fe/core/converters/controls/Common/Table";
import {
...
function getSectionFromAnnotation(facet: FacetTypes, converterContext: ConverterContext): ObjectPageSection {
	const sectionID = getSectionID({ Facet: facet });
...
export const getHeaderActions = function (converterContext: ConverterContext): CombinedAction {
	...
	const headerActions = insertCustomElements(aAnnotationHeaderActions, manifestActions.actions, {
...
export const getFooterActions = function (converterContext: ConverterContext): CombinedAction {
	...
	const footerActions = insertCustomElements(aAnnotationFooterActions, manifestActions.actions, {
		isNavigable: "overwrite",
		enabled: "overwrite",
		visible: "overwrite",
		defaultValuesExtensionFunction: "overwrite",
...
function _setGridTableWithMixFacetsInformation(subSection: DataVisualizationSubSection): void {
	if (subSection?.content) {
		if (subSection.content.length === 1) {
			if (
				((subSection.content[0] as DataVisualizationSubSection).presentation?.visualizations[0] as TableVisualization).control
					.type === "GridTable"
			) {
				(
					(subSection.content[0] as DataVisualizationSubSection).presentation?.visualizations[0] as TableVisualization
				).control.rowCountMode = "Auto";
			}
		}
	}
}
...
export const convertPage = function (converterContext: ConverterContext): ObjectPageDefinition {
	...
	const primaryAction = getPrimaryAction(converterContext, headerActions.actions, footerActions.actions);
