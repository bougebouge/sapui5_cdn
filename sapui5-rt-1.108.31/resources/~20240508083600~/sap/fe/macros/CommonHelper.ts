...
const CommonHelper = {
	...
	isPropertyFilterable: function (property: any, oInterface: any, oDataField: any) {
		const oModel = oInterface.context.getModel(),
			sPropertyPath = oInterface.context.getPath(),
	 ...
	 */
	_getDraftAdministrativeDataType: function (oMetaModel: ODataMetaModel, sEntityType: string) {
	 ...
	 */
	getPopoverText: function (iContext: any, sEntityType: string) {
	...
	getDataFromTemplate: function (dataElement: Context) {
		...
		const connectedObject = dataElement.getObject(connectedDataPath) as any;
...
(CommonHelper.isPropertyFilterable as any).requiresIContext = true;
(CommonHelper.getPopoverText as any).requiresIContext = true;
