...
oFF.XDateFormatter.prototype.applyPattern = function(format)
{
...
oFF.XDateRange.s_timeInMillis = 0;
oFF.XDateRange.create = function()
{
	return new oFF.XDateRange();
};
...
{
	var clone = oFF.XDateRange.create();
...
oFF.XDateRange.prototype.compute = function()
{
	if (oFF.isNull(this.m_from) || oFF.isNull(this.m_to))
	{
		...
		{
			var timeInMillis = oFF.XDateRange.s_timeInMillis > 0 ? oFF.XDateRange.s_timeInMillis : oFF.XSystemUtils.getCurrentTimeInMilliseconds();
			this.m_from = oFF.XGregorianCalendar.createWithTimeZone(timeZone);
...
oFF.XNumberFormatter = {
	...
	formatDoubleToStringUsingSettingsWithCurrency:function(value, numberFormatterSettings, currencyText, scaleTexts)
	{
		...
		{
			var currencyPresentation = currencyText;
			...
			var scaleText = oFF.XNumberFormatter.getScaleText(numberFormatterSettings.getScaleFactor(), numberFormatterSettings.getScaleFormat(), scaleTexts);
			...
			{
				case 12:
					return oFF.XStringUtils.concatenate3(formattedValue, scaleText, currencyPresentation);

				case 21:
					return oFF.XStringUtils.concatenate3(formattedValue, currencyPresentation, scaleText);

				case 102:
					return oFF.XStringUtils.concatenate3(scaleText, formattedValue, currencyPresentation);

				case 120:
					return oFF.XStringUtils.concatenate3(currencyPresentation, formattedValue, scaleText);

				case 201:
					return oFF.XStringUtils.concatenate3(scaleText, currencyPresentation, formattedValue);

				case 210:
					return oFF.XStringUtils.concatenate3(currencyPresentation, scaleText, formattedValue);
	...
	formatDouble:function(value, numberFormatterSettings)
	{
		...
		if (numberFormatterSettings.getScaleFactor() !== null)
		{
			...
			if (scaleFactor < 0)
			{
				valueString = oFF.XNumberFormatter.moveDecimalRight(valueString, -numberFormatterSettings.getScaleFactor().getInteger());
			}
			else if (scaleFactor > 0)
			{
				valueString = oFF.XNumberFormatter.moveDecimalLeft(valueString, numberFormatterSettings.getScaleFactor().getInteger());
			}
		}
	...
	getScaleText:function(scaling, scaleFormat, scaleTexts)
	{
		...
		if (scalingFactor < 0)
		{
			...
			return oFF.XStringUtils.concatenate3("(0.", oFF.XStringUtils.leftPad("1", "0", numZeros), ")");
		}
		...
		switch (scalingFactor)
		{
			...
			default:
				...
				return oFF.XStringUtils.concatenate3("(", billionsString, "b)");
		}
	}
};
