...
oFF.QInARepoUtils = {
	...
	importFilterTimeRange:function(inaStructure, fixedTimeRangeFilter)
	{
	...
	exportTimeRangeFilter:function(inaStructure, timeRangeFilter)
	{
...
oFF.QInARepository.prototype.extendCustom = function(exporter, modelComponent, inaStructure)
{
	...
	if (exporter.mode !== oFF.QModelFormat.INA_REPOSITORY_DELTA || inaStructure.hasElements())
	{
		...
		if (oFF.XCollectionUtils.hasElements(changedProperties) && repoDeltaFeatureActivated)
		{
		...
		var changedPropertiesInDetail = modelComponent.getChangedPropertiesInDetail();
		if (oFF.XCollectionUtils.hasElements(changedPropertiesInDetail) && repoDeltaFeatureActivated)
		{
		...
		if (repoDeltaFeatureActivated && inaStructure.hasElements())
		{
...
oFF.QInARepoFilterDynamicTimeRegularRange.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	...
	oFF.QInARepoUtils.importFilterTimeRange(inaStructure, dynamicTimeRangeFilter);
...
oFF.QInARepoFilterDynamicTimeRegularRange.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	...
	oFF.QInARepoUtils.exportTimeRangeFilter(inaStructure, dynamicTimeRangeFilter);
...
oFF.QInARepoFilterDynamicTimeToDateRange.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	...
	oFF.QInARepoUtils.importFilterTimeRange(inaStructure, toDateRangeFilter);
	oFF.QInARepoUtils.importFilterShiftableTimeRange(inaStructure, toDateRangeFilter);
	toDateRangeFilter.setMemberValue(inaStructure.getStringByKey("Member"));
	var lowValueType = inaStructure.getStringByKey("LowValueType");
	if (oFF.notNull(lowValueType))
	{
...
oFF.QInARepoFilterDynamicTimeToDateRange.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	...
	oFF.QInARepoUtils.exportTimeRangeFilter(inaStructure, toDateRangeFilter);
	oFF.QInARepoUtils.exportShiftableTimeRangeFilter(inaStructure, toDateRangeFilter);
	inaStructure.putString("Member", toDateRangeFilter.getMemberValue());
	return oFF.QInARepository.prototype.exportComponentWithStructure.call( this , exporter, modelComponent, inaStructure, flags);
};
...
oFF.QInARepoFilterFixedTimeRange.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	...
	oFF.QInARepoUtils.importFilterTimeRange(inaStructure, fixedTimeRangeFilter);
...
oFF.QInARepoFilterFixedTimeRange.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	...
	oFF.QInARepoUtils.exportTimeRangeFilter(inaStructure, fixedTimeRangeFilter);
		...
		else if (importer.mode !== oFF.QModelFormat.INA_REPOSITORY_DELTA || inaStructure.hasElements())
		{
			...
			if (oFF.XStringUtils.isNotNullAndNotEmpty(hierarchyName))
			{
				...
				if (oFF.notNull(hierarchy))
				{
					...
					var isActive = isNameVariable ? false : inaStructure.getBooleanByKeyExt("HierarchyActive", isHierarchyActiveDefaultValue);
					dimension.setHierarchyActive(isActive);
					dimension.setSelectorHierarchyActive(isActive);
					var queryManagerBase = dimension.getQueryManager();
...
oFF.QInARepoHierarchy.prototype.importHierarchyName = function(inaStructure, dimension, isNameVariable)
{
	...
	var queryManagerBase = dimension.getQueryManager();
	if (oFF.notNull(queryManagerBase) && queryManagerBase.isExecuteAutoVariableSubmit())
	{
	...
	if (oFF.notNull(allStructureMembers))
	{
		...
		for (var i = 0; i < len; i++)
		{
			...
			if (structureMember.isModelDefinedMember())
			{
				...
				{
					if (dimension.getDimensionType() === oFF.DimensionType.ACCOUNT || exporter.getOriginalMode() === oFF.QModelFormat.INA_REPOSITORY_DELTA)
					{
						continue;
					}
...
oFF.QInARepoTableDefinition.importPathAndAxis = function(inaStruct, pathReference, queryModel)
{
	pathReference.setAxisType(oFF.AxisType.lookup(inaStruct.getStringByKey("AxisType")));
	var pathList = oFF.XList.create();
	var pathsPrList = inaStruct.getListByKey("ReferencePath");
	for (var j = 0; j < pathsPrList.size(); j++)
	{
		var pathPrElement = pathsPrList.getStructureAt(j);
		pathList.add(oFF.QFactory.createDrillPathElement(queryModel, pathPrElement.getStringByKey("MemberName"), queryModel.getDimensionByName(pathPrElement.getStringByKey("DimensionName"))));
	}
	pathReference.setPathOnAxis(pathList);
};
oFF.QInARepoTableDefinition.exportPathAndAxis = function(inaStructure, axisPathReference)
{
	inaStructure.putString("Name", axisPathReference.getName());
	inaStructure.putString("Text", axisPathReference.getText());
	if (axisPathReference.getAxisType() !== null)
	{
		inaStructure.putString("AxisType", axisPathReference.getAxisType().getName());
	}
	var referencePath = inaStructure.putNewList("ReferencePath");
	var pathOnAxis = axisPathReference.getPathOnAxis();
	for (var j = 0; j < pathOnAxis.size(); j++)
	{
		var pathElement = pathOnAxis.get(j);
		var pathElementStructure = referencePath.addNewStructure();
		pathElementStructure.putString("DimensionName", pathElement.getDimension().getName());
		pathElementStructure.putString("MemberName", pathElement.getName());
	}
};
...
oFF.QInARepoTableDefinition.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	if (oFF.notNull(modelComponent) && oFF.notNull(inaStructure) && modelComponent.getComponentType() === oFF.OlapComponentType.VISUALIZATION_TABLE_DEFINITION)
	{
		...
		var queryModel = tableDefinition.getQueryModel();
		...
		{
			for (i = 0; i < formattingStyleList.size(); i++)
			{
				...
				formattingStyle.setCellFormattingScope(oFF.CellFormattingScope.lookup(formattingStyleStruct.getStringByKey("Scope")));
			}
		}
		...
		var tupleFormatList = inaStructure.getListByKey("TupleFormatting");
		if (oFF.notNull(tupleFormatList))
		{
			tableDefinition.clearTupleFormattings();
		}
		if (oFF.XCollectionUtils.hasElements(tupleFormatList))
		{
			for (i = 0; i < tupleFormatList.size(); i++)
			{
				var tupleFormatStruct = tupleFormatList.getStructureAt(i);
				var tupleFormat = tableDefinition.addNewTupleFormatting(tupleFormatStruct.getStringByKey("Name"), tupleFormatStruct.getStringByKey("Text"));
				tupleFormat.setFormattingStyleName(tupleFormatStruct.getStringByKey("Style"));
				oFF.QInARepoTableDefinition.importPathAndAxis(tupleFormatStruct, tupleFormat, queryModel);
			}
		}
		var insertList = inaStructure.getListByKey("InsertedCells");
		if (oFF.XCollectionUtils.hasElements(insertList))
		{
			tableDefinition.clearInsertedCells();
			for (i = 0; i < insertList.size(); i++)
			{
				var insertStruct = insertList.getStructureAt(i);
				var insert = tableDefinition.addNewInsertedCell(insertStruct.getStringByKey("Name"), insertStruct.getStringByKey("Text"));
				insert.setInsertOffset(insertStruct.getIntegerByKey("InsertPositionOffset"));
				oFF.QInARepoTableDefinition.importPathAndAxis(insertStruct, insert, queryModel);
			}
		}
		var hiddenCellList = inaStructure.getListByKey("HiddenCells");
		if (oFF.XCollectionUtils.hasElements(hiddenCellList))
		{
			tableDefinition.clearHiddenCells();
			for (i = 0; i < hiddenCellList.size(); i++)
			{
				var hiddenStruct = hiddenCellList.getStructureAt(i);
				var hide = tableDefinition.addNewHiddenCells(hiddenStruct.getStringByKey("Name"), hiddenStruct.getStringByKey("Text"));
				oFF.QInARepoTableDefinition.importPathAndAxis(hiddenStruct, hide, queryModel);
			}
		}
		...
		tableDefinition.setStripeDataColumns(inaStructure.getBooleanByKey("StripeDataColumns"));
		tableDefinition.setStripeDataRows(inaStructure.getBooleanByKey("StripeDataRows"));
		tableDefinition.setFreezeHeaderRows(inaStructure.getBooleanByKey("FreezeHeaderRows"));
		tableDefinition.setFreezeHeaderColumns(inaStructure.getBooleanByKey("FreezeHeaderColumns"));
		...
		tableDefinition.setShowFreezeLines(inaStructure.getBooleanByKey("ShowFreezeLines"));
		tableDefinition.setShowGrid(inaStructure.getBooleanByKey("ShowGrid"));
		tableDefinition.setShowTableTitle(inaStructure.getBooleanByKeyExt("ShowTableTitle", true));
		tableDefinition.setShowTableDetails(inaStructure.getBooleanByKeyExt("ShowTableDetails", false));
		tableDefinition.setShowSubTitle(inaStructure.getBooleanByKeyExt("ShowSubtitle", false));
		tableDefinition.setShowFormulas(inaStructure.getBooleanByKey("ShowFormulas"));
		tableDefinition.setShowDimensionTitles(inaStructure.getBooleanByKeyExt("ShowDimensionTitles", true));
		tableDefinition.setRepetitiveMemberNames(inaStructure.getBooleanByKey("ShowRepetitiveMemberNames"));
		tableDefinition.setMergeRepetitiveMemberCells(inaStructure.getBooleanByKey("MergeRepetitiveMemberCells"));
		tableDefinition.setShowCoordinateHeader(inaStructure.getBooleanByKey("ShowCoordinateHeader"));
		tableDefinition.setShowReferences(inaStructure.getBooleanByKey("ShowReferences"));
...
oFF.QInARepoTableDefinition.prototype.importStyledLine = function(styledLine, importStructure)
{
	...
	styledLine.setPatternWidth(importStructure.getStringByKey("PatternWidth"));
...
oFF.QInARepoTableDefinition.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	if (oFF.notNull(inaStructure) && oFF.notNull(modelComponent))
	{
		...
		if (oFF.XCollectionUtils.hasElements(formattingStyles))
		{
			...
			for (i = 0; i < formattingStyles.size(); i++)
			{
				...
				if (formattingStyle.getCellFormattingScope() !== null)
				{
					formattingStyleStruct.putString("Scope", formattingStyle.getCellFormattingScope().getName());
				}
			}
		}
		...
		if (oFF.XCollectionUtils.hasElements(dataPointStyles))
		{
			...
			for (i = 0; i < dataPointStyles.size(); i++)
			{
		...
		var tupleFormattings = tableDefinition.getTupleFormattings();
		if (oFF.XCollectionUtils.hasElements(tupleFormattings))
		{
			var tupleFormattingList = inaStructure.putNewList("TupleFormatting");
			for (i = 0; i < tupleFormattings.size(); i++)
			{
				var tupleFormattingStructure = tupleFormattingList.addNewStructure();
				var tupleFormatting = tupleFormattings.get(i);
				tupleFormattingStructure.putString("Style", tupleFormatting.getFormattingStyleName());
				oFF.QInARepoTableDefinition.exportPathAndAxis(tupleFormattingStructure, tupleFormatting);
			}
		}
		var insertedCells = tableDefinition.getInsertedCells();
		if (oFF.XCollectionUtils.hasElements(insertedCells))
		{
			var insertList = inaStructure.putNewList("InsertedCells");
			for (i = 0; i < insertedCells.size(); i++)
			{
				var insertStructure = insertList.addNewStructure();
				var insert = insertedCells.get(i);
				insertStructure.putInteger("InsertPositionOffset", insert.getInsertOffset());
				oFF.QInARepoTableDefinition.exportPathAndAxis(insertStructure, insert);
			}
		}
		var hiddenCells = tableDefinition.getHiddenCells();
		if (oFF.XCollectionUtils.hasElements(hiddenCells))
		{
			var hideList = inaStructure.putNewList("HiddenCells");
			for (i = 0; i < hiddenCells.size(); i++)
			{
				var hideStructure = hideList.addNewStructure();
				var hiddenCell = hiddenCells.get(i);
				oFF.QInARepoTableDefinition.exportPathAndAxis(hideStructure, hiddenCell);
			}
		}
		var keys;
		var key;
		...
		inaStructure.putBoolean("StripeDataRows", tableDefinition.isStripeDataRows());
		inaStructure.putBoolean("StripeDataColumns", tableDefinition.isStripeDataColumns());
		inaStructure.putBoolean("FreezeHeaderRows", tableDefinition.isFreezeHeaderRows());
		inaStructure.putBoolean("FreezeHeaderColumns", tableDefinition.isFreezeHeaderColumns());
		...
		inaStructure.putBoolean("ShowFreezeLines", tableDefinition.isShowFreezeLines());
		inaStructure.putBoolean("ShowGrid", tableDefinition.isShowGrid());
		inaStructure.putBoolean("ShowTableTitle", tableDefinition.isShowTableTitle());
		inaStructure.putBoolean("ShowTableDetails", tableDefinition.isShowTableDetails());
		inaStructure.putBoolean("ShowSubtitle", tableDefinition.isShowSubTitle());
		inaStructure.putBoolean("ShowFormulas", tableDefinition.isShowFormulas());
		inaStructure.putBoolean("ShowDimensionTitles", tableDefinition.isShowDimensionTitles());
		inaStructure.putBoolean("ShowRepetitiveMemberNames", tableDefinition.isRepetitiveMemberNames());
		inaStructure.putBoolean("MergeRepetitiveMemberCells", tableDefinition.isMergeRepetitiveMemberCells());
		inaStructure.putBoolean("ShowCoordinateHeader", tableDefinition.isShowCoordinateHeader());
		inaStructure.putBoolean("ShowReferences", tableDefinition.isShowReferences());
...
oFF.QInARepoTableDefinition.prototype.exportStyledLine = function(styledLine, exportStructure)
{
	...
	exportStructure.putStringNotNullAndNotEmpty("PatternWidth", styledLine.getPatternWidth());
