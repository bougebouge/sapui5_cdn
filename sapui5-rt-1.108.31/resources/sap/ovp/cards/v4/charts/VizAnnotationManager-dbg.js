...
sap.ui.define([
            ...
            if (oMetadata && oMetadata[tmp]) {
                if (oMetadata[tmp]["annotations"] && oMetadata[tmp]["annotations"]["@" + textKeyV4]) {
                    ...
                    if (
                        ...
                        if (aParts.length > 1) {
                            sExpand = getNavigationPrefix(oMetaModel, oAnnotations, textPath);
                            if (sExpand !== "") {
                                dimensionsList.push(textPath);
                                aExpand.push(sExpand);
                                aTempAssociationKeyArr = getAssociationKeys(oMetaModel, oAnnotations, textPath);
    ...
    function checkForecastMeasure(aMeasure, entityTypeObject) {
        ...
        if (aMeasure.DataPoint && aMeasure.DataPoint.$AnnotationPath) {
            var oDatapoint = entityTypeObject[aMeasure.DataPoint.$AnnotationPath];
            if (
                oDatapoint &&
    ...
    function checkNumberFormat(minValue, val, entityTypeObject) {
        if (val && val.DataPoint && val.DataPoint.$AnnotationPath) {
            var sAnnotationPath =  val.DataPoint.$AnnotationPath,
            oDataPoint = entityTypeObject[sAnnotationPath];
            ...
            if (
                ...
                fractionDigitsVal = fractionDigits.NumberOfFractionalDigits;
    ...
    function getMaxScaleFactor(maxScaleValue, val, entityTypeObject) {
        if (val && val.DataPoint && val.DataPoint.$AnnotationPath) {
            var oDataPoint = entityTypeObject[val.DataPoint.$AnnotationPath];
