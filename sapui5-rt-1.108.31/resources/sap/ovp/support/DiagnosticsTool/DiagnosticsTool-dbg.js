sap.ui.define([
    ...
    // ------------------------------------ Variables ------------------------------------
    var sPluginId = "sapUiSupportFioriElementsPluginOVP"; // plugin id
    ...
    function getMethods(oSupportStub) {
        var oPlugin, // this plugin object
            ...
            oEventBus = Core.getEventBus(), // global event bus instance
            ...
            try {
                if (oUI5Version && CommonMethods.hasObjectContent(oUI5Version)) {
                    fnAddStringToData(
                        "OpenUI5 Version",
                        iSortOrder,
                        oUI5Version.version + " (built at " + fnFormatDate(oUI5Version.buildTimestamp) + ")"
                    );
                    return true;
                } else {
                ...
                fnAddStringToData(
                    "OpenUI5 Version",
                    iSortOrder,
                    oUI5Version && oUI5Version.version +
                    ', detailed UI5 version info is not available! Possible reason: missing file "sap-ui-version.json"'
                );
                return true;
            }
        }
        ...
        function fnAddApplicationName(iSortOrder) {
            ...
            if (sApplicationName) {
                ...
                fnAddStringToData(
                    "Application URL",
                    iSortOrder,
                    "ERROR: Could not extract application name (#semanticObject-action) from URL!"
                );
                return false;
            }
        }
        ...
        function fnAddManifestLink(iSortOrder) {
            if (sManifestPath && sAbsoluteManifestURL) {
                ...
                fnAddStringToData(
                    "Manifest",
                    iSortOrder,
                    "ERROR: Could not generate link to manifest.json! Possible reason: The application did not finish loading or is not a Fiori Elements application."
                );
                return false;
            }
        }
        ...
        function fnAddFioriID(iSortOrder) {
            ...
            if (aFioriIDs && Array.isArray(aFioriIDs) && aFioriIDs.length > 0) {
                fnAddStringToData(
                    aFioriIDs.length > 1 ? "Fiori IDs" : "Fiori ID",
                    iSortOrder,
                    CommonMethods.concatStrings(aFioriIDs)
                );
                return true;
            }
        ...
        function fnAddApplicationComponent(iSortOrder) {
            ...
            if (sApplicationComponent) {
                ...
                fnAddStringToData(
                    ...
                    iSortOrder,
                    "ERROR: Path /sap.app/ach not found in manifest.json! Possible reason: Invalid manifest.json"
                );
                return false;
            }
        }
        ...
        function fnAddApplicationID(iSortOrder) {
            ...
            if (sApplicationID) {
                ...
                fnAddStringToData(
                    "Application ID",
                    iSortOrder,
                    "ERROR: Path /sap.app/id not found in manifest.json! Possible reason: Invalid manifest.json"
                );
                return false;
            }
        }
        ...
        function fnAddFloorplanComponent(iSortOrder) {
            ...
            if (sFloorplan === CommonChecks.mFloorplans.UNKNOWN) {
                fnAddStringToData(
                    "Floorplan Component (ACH)",
                    iSortOrder,
                    CommonChecks.getTicketComponentForFloorplan(sFloorplan) +
                    " (ERROR: Unknown floorplan! Possible reason: Invalid manifest.json)"
                );
                return false;
            } else {
                fnAddStringToData(
                    "Floorplan Component (ACH)",
                    iSortOrder,
                    CommonChecks.getTicketComponentForFloorplan(sFloorplan) + " (" + sFloorplan + ")"
                );
                return true;
            }
        }
        ...
        function fnAddODataServiceMetadataLink(sDataSourceName, iSortOrder) {
            ...
            if (
                ...
                fnAddStringToData(
                    ...
                    "ERROR: Data source " +
                    sDataSourceName +
                    " not found at /sap.app/dataSources/" +
                    sDataSourceName +
                    " in manifest.json! Possible reason: Invalid manifest.json"
                );
                return false;
            }
            ...
            if (!oManifest["sap.app"].dataSources[sDataSourceName].uri) {
                fnAddStringToData(
                    "OData Service Metadata",
                    iSortOrder,
                    "ERROR: Data source URI not found at /sap.app/dataSources/" +
                    sDataSourceName +
                    "/uri in manifest.json! Possible reason: Invalid manifest.json"
                );
                return false;
            }
        ...
        function fnAddAnnotationsLinks(sDataSourceName, iSortOrder) {
            ...
            if (
                !(
                    oManifest["sap.app"].dataSources[sDataSourceName].settings &&
                ...
                fnAddStringToData(
                    ...
                    "ERROR: Data source " +
                    sDataSourceName +
                    " has no annotations at /sap.app/dataSources/" +
                    sDataSourceName +
                    "/settings/annotations in manifest.json! Possible reason: Invalid manifest.json"
                );
                return false;
            }
            ...
            for (var iKey in aDataSourceAnnotations) {
                ...
                if (oManifest["sap.app"].dataSources[sSourceName]) {
                    ...
                    fnAddLinkToData(
                        sName,
                        iSortOrder,
                        oManifest["sap.app"].dataSources[sSourceName].uri,
                        sPrefix + oManifest["sap.app"].dataSources[sSourceName].uri
                    );
                }
            }
            return true;
        ...
        function fnAddDataSources(iSortOrder) {
            ...
            if (!(oManifest["sap.ui5"] && oManifest["sap.ui5"].models)) {
                fnAddStringToData(
                    "Data Sources",
                    iSortOrder,
                    "ERROR: Path /sap.ui5/models not found in manifest.json! Possible reason: Invalid manifest.json"
                );
                return;
            }
            ...
            if (aDataSources.length === 0) {
                fnAddStringToData(
                    "Data Sources",
                    iSortOrder,
                    "ERROR: No models with data sources found in manifest.json! Possible reason: Invalid manifest.json"
                );
                return;
            }
            ...
            for (var iGroupIndex in aDataSources) {
                ...
                if (!(oManifest["sap.app"] && oManifest["sap.app"].dataSources)) {
                    fnAddStringToData(
                        "Data Sources",
                        iSortOrder,
                        "ERROR: No data sources found at /sap.app/dataSources in manifest.json! Possible reason: Invalid manifest.json"
                    );
                    return;
                }
                ...
                if (!oManifest["sap.app"].dataSources[aDataSources[iGroupIndex].dataSource]) {
                    fnAddStringToData(
                        ...
                        iSortOrder,
                        "ERROR: Data source " +
                        aDataSources[iGroupIndex].dataSource +
                        " not found at /sap.app/dataSources/" +
                        aDataSources[iGroupIndex].dataSource +
                        " in manifest.json! Possible reason: Invalid manifest.json"
                    );
                ...
                fnAddGroupHeaderToData(
                    CommonMethods.concatStrings(aDataSources[iGroupIndex].models),
                    increaseSubSort(iSortOrder)
                );
        ...
        function fnGetView(sId) {
            var oView = Core.byId(sId);
            if (oView) {
                return oView;
            } else {
                return new XMLView(sId, {
        ...
        function addContent(bRecoveryMode) {
            ...
            if (bRecoveryMode && oManifest && CommonMethods.hasObjectContent(oManifest)) {
                ...
                jQuery
                    .when(CommonMethods.getFileFromURI(sAbsoluteManifestURL))
                    ...
                    .fail(function () {
                        fnAddStringToData(
                            "Manifest",
                            3,
                            "ERROR: Could not access manifest.json even though link could be generated! Possible reason: missing permission to access file."
                        );
                    })
                    .always(function () {
                        fnTriggerSetData();
        ...
        function fnOnGetData() {
            ...
            if (sApplicationStatus === CommonMethods.mApplicationStatus.LOADING) {
                ...
                if (oResult && CommonMethods.hasObjectContent(oResult)) {
                    ...
                    fnDisplayError(
                        "Could not load any data because manifest and component of current application are unknown!"
                    );
                    fnTriggerShowDataRefreshed();
                    return;
                }
        ...
        function fnHandleBusyState(sChannel, sEventName) {
            ...
            if (
                sEventName === "ViewRenderingStarted" ||
                (!sEventName && CommonMethods.getApplicationStatus() === CommonMethods.mApplicationStatus.LOADING)
            ) {
                ...
                if (!oIntervalTrigger) {
                    iTimeLeft = iTimeout;
                    oIntervalTrigger = new IntervalTrigger(1000);
