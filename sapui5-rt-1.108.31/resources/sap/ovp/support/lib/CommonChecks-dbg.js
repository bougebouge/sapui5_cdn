...
sap.ui.define([
    ...
    function fnGetUI5VersionInfo() {
        var oApp = CommonUtils.getApp();
        if (oApp) {
            var oOVPConfig = oApp.getOwnerComponent().oOvpConfig;
            return oOVPConfig.sapUICoreVersionInfo;
        }
    }
    ...
    function fnGetRegistrationIDsByManifest(oManifest) {
        return oManifest &&
            oManifest["sap.fiori"] &&
            oManifest["sap.fiori"].registrationIds &&
    ...
    function fnGetManifestByStructure() {
        ...
        if ($OVPWrapper && $OVPWrapper.length && $OVPWrapper.length > 0) {
            ...
            if (
                ...
                $XMLView[0] &&
                $XMLView[0].id &&
                Core.byId($XMLView[0].id) &&
                Core.byId($XMLView[0].id).getController() &&
                Core.byId($XMLView[0].id).getController().getOwnerComponent() &&
                Core.byId($XMLView[0].id).getController().getOwnerComponent().getMetadata() &&
                Core.byId($XMLView[0].id).getController().getOwnerComponent().getMetadata().getManifest()
            ) {
                oManifest = Core
                    .byId($XMLView[0].id)
                    .getController()
            ...
            for (var i = 0; i < $XMLView.length; i++) {
                if ($XMLView[i].id.indexOf(sPattern) !== -1) {
                    if (
                        Core.byId($XMLView[i].id) &&
                        Core.byId($XMLView[i].id).getController() &&
                        Core.byId($XMLView[i].id).getController().getOwnerComponent() &&
                        Core.byId($XMLView[i].id).getController().getOwnerComponent().getAppComponent() &&
                            Core
                            .byId($XMLView[i].id)
                            .getController()
                            .getOwnerComponent()
