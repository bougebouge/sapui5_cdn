...
const Delegate = {
	...
	createLayout: async function (mPropertyBag: {
		...
		const oMetaModelContext: Context = oMetaModel.getMetaContext(sBindingPath);
		const oPropertyContext: Context = oMetaModel.createBindingContext(`${sBindingPath}/${mPropertyBag.bindingPath}`);
			...
			if (oModel) {
				if (oModel.isA("sap.ui.model.odata.v4.ODataModel")) {
					...
					if (sBindingContextPath) {
						...
						const mODataEntityType = oMetaModelContext.getObject((oMetaModelContextObject as any).$Type);
						return _convertMetadataToDelegateFormat(
							mODataEntityType,
							(oMetaModelContextObject as any).$Type,
							oMetaModel,
							oElement,
							sAggregationName
						);
