	...
	if (property) {
		switch (property.$Type) {
			...
			case UIAnnotationTypes.DataField:
				if (property.annotations && property.annotations?.UI?.Hidden?.valueOf() === true) {
					...
					return true;
				} else if (property.Value.$target && property.Value?.$target?.annotations?.UI?.Hidden?.valueOf() === true) {
...
export function isAnnotationFieldStaticallyHidden(annotationProperty: DataFieldForAnnotationTypes) {
	...
	switch (target) {
		...
		case UIAnnotationTerms.FieldGroup:
			if (annotationProperty) {
				(annotationProperty as any).Target.$target.Data.forEach((field: DataFieldAbstractTypes) =>
