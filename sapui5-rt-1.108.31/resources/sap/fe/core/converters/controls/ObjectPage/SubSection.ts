...
import type { BaseAction, CombinedAction, ConverterAction, CustomAction } from "sap/fe/core/converters/controls/Common/Action";
import {
	ButtonType,
...
function addFormMenuActions(actions: ConverterAction[], facetDefinition: FacetTypes, converterContext: ConverterContext): CombinedAction {
	const hiddenActions: BaseAction[] = getFormHiddenActions(facetDefinition, converterContext) || [],
		...
		formAllActions = insertCustomElements(actions, manifestActions.actions, { command: "overwrite" });
	return {
...
export function createSubSection(
	...
	const subSectionID = getSubSectionID({ Facet: facetDefinition });
	...
	const isDynamicExpression =
		isVisible !== undefined && typeof isVisible === "string" && isVisible.indexOf("{=") === 0 && !isPathExpression(oHiddenAnnotation);
			...
			if (!facetDefinition.Target.$target) {
				...
				switch (facetDefinition.Target.$target.term) {
					...
					case UIAnnotationTerms.SelectionPresentationVariant:
						...
						const showTitle = isSubsectionTitleShown(hasSingleContent, subSection.title, controlTitle);
						...
						const dataVisualizationSubSection: DataVisualizationSubSection = {
							...
							showTitle: showTitle, // This is used on the ObjectPageSubSection
...
function isSubsectionTitleShown(
	hasSingleContent: boolean,
	subSectionTitle: CompiledBindingToolkitExpression,
	controlTitle: string
): boolean {
		...
		switch (dataField.$Type) {
			case UIAnnotationTypes.DataFieldForIntentBasedNavigation:
				...
				actionReducer.push({
					...
					id: getFormID({ Facet: facetDefinition }, dataField),
