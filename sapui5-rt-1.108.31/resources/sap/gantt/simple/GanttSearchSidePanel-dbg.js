sap.ui.define([
    ...
    "sap/gantt/library"
], function (Control, Core, FindAndSelectUtils, NavigationListItem, NavigationList, library) {
    'use strict';
    ...
    GanttSearchSidePanel.prototype.onAfterRendering = function () {
        ...
        // Add event listeners to Side Panel
    ...
    GanttSearchSidePanel.prototype.setContainerHeight = function () {
        ...
        var iPageHeight = this.getParent().getUIArea().getContent()[0].getDomRef().getBoundingClientRect().bottom;
        var iBufferHeight = this.getParent().getEnableStatusBar() ? 64 : 32;
        var containerHeight = iPageHeight - (containerTop + iBufferHeight);
        this._oSearchSidePanel.getItems()[3].setHeight(containerHeight + "px");
        this._oSearchSidePanel.getItems()[3].rerender();
    };
    ...
    GanttSearchSidePanel.prototype._updateContent = function () {
        ...
        this._oSearchSidePanel.getItems()[2].rerender();
        this._oSearchSidePanel.getItems()[3].rerender();
        this.addEventListener();
    };
    ...
    GanttSearchSidePanel.prototype.addEventListener = function () {
        var oDomRef = this._oSearchSidePanel.getItems()[3].getContent()[0]._itemNavigation.oDomRef;
        ...
        oDomRef.addEventListener("keydown", function(oEvent) {
            ...
            if (oEvent.code === "ArrowDown") {
                if (FindAndSelectUtils._selectedIndex === FindAndSelectUtils._searchResultsCount - 1) {
                    FindAndSelectUtils._selectedIndex = parseInt(currentElementID.replace(/\D/g,''), 10);
                } else {
                    FindAndSelectUtils._selectedIndex = parseInt(currentElementID.replace(/\D/g,''), 10) + 1;
                }
        ...
        oDomRef.addEventListener("click", function() {
            FindAndSelectUtils._selectedIndex = parseInt(document.activeElement.id.replace(/\D/g,''), 10);
            this._oSearchSidePanel.getItems()[3].getContent()[0].setSelectedItem("listItem" + FindAndSelectUtils._selectedIndex);
