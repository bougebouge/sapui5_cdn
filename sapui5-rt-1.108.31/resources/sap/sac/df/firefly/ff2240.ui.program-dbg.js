...
{
	if (this.getProgram() !== null && this.getProgram().getContainerAfterCloseProcedure() !== null)
	{
		this.getProgram().getContainerAfterCloseProcedure()();
	}
};
oFF.UiPrgContainer.prototype.notifyContainerOpened = function()
{
	if (this.getProgram() !== null && this.getProgram().getContainerAfterOpenProcedure() !== null)
	{
		this.getProgram().getContainerAfterOpenProcedure()();
	}
};
...
oFF.DfUiProgram.prototype.renderUi = function(genesis)
{
	...
	if (this.shouldWrapContentInPage())
	{
		...
		{
			this.m_autoCreatedMenuBar = genesis.newControl(oFF.UiType.OVERFLOW_TOOLBAR);
			this.m_autoCreatedMenuBar.setTag(oFF.DfUiProgram.PRG_MENU_BAR_NAME);
			this.m_autoCreatedMenuBar.setToolbarDesign(oFF.UiToolbarDesign.SOLID);
...
{
	if (this.getGenesis() !== null && this.getGenesis().getRoot() !== null)
	{
		this.getGenesis().getRoot().setBusy(busy);
	}
};
oFF.DfUiProgram.prototype.isBusy = function()
{
	...
	if (this.getGenesis() !== null && this.getGenesis().getRoot() !== null)
	{
		isBusy = this.getGenesis().getRoot().isBusy();
	}
	return isBusy;
};
...
oFF.UiPrgContainerGenericWindow.prototype.createWindowMenuIfAvailable = function()
{
	...
	if (oFF.notNull(tmpMenuItems) && tmpMenuItems.size() > 0)
	{
		this.getWindow().registerOnMenuPress(oFF.UiLambdaMenuPressListener.create( function(controlEvent){
			var posX = controlEvent.getParameters().getIntegerByKeyExt(oFF.UiControlEvent.PARAM_MENU_X, -1);
			var posy = controlEvent.getParameters().getIntegerByKeyExt(oFF.UiControlEvent.PARAM_MENU_Y, -1);
...
oFF.UiPrgContainerTerminal.prototype.openAndRun = function(uiManager)
{
	...
	this.setContainerNameAndCssClass(tmpTerminalWindow);
...
oFF.UiPrgContainerWindow.prototype.openAndRun = function(uiManager)
{
	...
	this.setContainerNameAndCssClass(tmpWindow);
	this.setContainerWindow(tmpWindow);
	uiProgram.evalArguments();
	uiProgram.initializeProgram();
	tmpWindow.open();
	var innerGenesis = oFF.UiGenesis.create(tmpWindow);
	uiProgram.renderUi(innerGenesis);
};
