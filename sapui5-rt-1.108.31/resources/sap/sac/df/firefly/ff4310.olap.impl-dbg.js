...
oFF.BlendingSortUtils = {
	...
	addSortByDimension:function(blendableQueryManager, queryModel, sortingManager, sort)
	{
		...
		var dimension = keyField.getDimension();
...
oFF.BlendingUtils = {
	...
	convertToBlendedFormula:function(formulaItem, parentMeasure)
	{
		...
		else if (formulaItem.getOlapComponentType() === oFF.OlapComponentType.FORMULA_ITEM_MEMBER)
		{
			...
			if (oFF.notNull(dimensionContext))
			{
				...
				var blendedMember = oFF.BlendingUtils.getMatchingBlendedMemberFromLeafMember(parentMeasure.getQueryModel(), structureMember);
	...
	getMatchingLeafMemberFromBlendedMember:function(queryModel, blendedMemberName, isPrimaryCalculationDimension)
	{
		...
		if (queryManager.getMode() === oFF.QueryManagerMode.BLENDING)
		{
			...
			for (var i = 0; i < blendingSources.size(); i++)
			{
				...
				{
					var memberNameWithoutPrefix = oFF.XString.replace(blendedMemberName, prefix, "");
					memberToFind = oFF.BlendingUtils.getMatchingLeafMemberFromBlendedMember(leafQueryModel, memberNameWithoutPrefix, isPrimaryCalculationDimension);
		...
		{
			var dimension = isPrimaryCalculationDimension ? queryModel.getPrimaryCalculationDimension() : queryModel.getSecondaryCalculationDimension();
	...
	{
			var allStructureMembers = queryModel.getPrimaryCalculationDimension().getAllStructureMembers();
		var runtimePlaceholderId = leafMember.getDimension().getRuntimePlaceholderIdByAlias(leafMember.getAliasName());
		var memberNameToMatch = oFF.notNull(runtimePlaceholderId) ? runtimePlaceholderId : leafMember.getName();
		for (var i = 0; i < allStructureMembers.size(); i++)
		{
			var structureMember = allStructureMembers.get(i);
			var memberNameIndex = oFF.XString.indexOf(structureMember.getName(), memberNameToMatch);
			if (memberNameIndex >= 0)
			{
				var leafMemberFound = oFF.BlendingUtils.getMatchingLeafMemberFromBlendedMember(queryModel, structureMember.getName(), true);
	...
	{
			var dimensionInList = oFF.XList.create();
		var rowDimensions = queryModel.getRowsAxis().getDimensions();
		for (var i = 0; i < rowDimensions.size(); i++)
		{
			var rowDimension = rowDimensions.get(i);
			if (!rowDimension.supportsBasicStructureMembers())
			{
				dimensionInList.add(rowDimension);
			}
		}
		var columnDimensions = queryModel.getColumnsAxis().getDimensions();
		for (var j = 0; j < columnDimensions.size(); j++)
		{
			var columnDimension = columnDimensions.get(j);
			if (!columnDimension.supportsBasicStructureMembers())
			{
				dimensionInList.add(columnDimension);
			}
		}
		var hasDimensionsInDrill = !dimensionInList.isEmpty();
		var primaryCalculationDimension = queryModel.getPrimaryCalculationDimension();
		var memberNames = oFF.XHashSetOfString.create();
		memberNames.addAll(oFF.QFilterUtil.getMemberNamesFromFilter(queryModel.getFilter().getDynamicFilter().getFilterRootElement(), primaryCalculationDimension.getName()));
		if (memberNames.isEmpty())
		{
			var linkedFilters = queryModel.getFilter().getLinkedFilters().getValuesAsReadOnlyList();
			for (var k = 0; k < linkedFilters.size(); k++)
			{
				var filterElement = linkedFilters.get(k).getFilterRootElement();
				memberNames.addAll(oFF.QFilterUtil.getMemberNamesFromFilter(filterElement, primaryCalculationDimension.getName()));
			}
		}
		var hasMeasureMemberInFilter = !memberNames.isEmpty();
		return hasDimensionsInDrill || (useEmptyAllMemberMeasureFilter ? true : hasMeasureMemberInFilter);
	},
	...
	{
			var leafDimension = null;
		for (var i = 0; i < leafQueryModels.size(); i++)
		{
			var leafQueryModel = leafQueryModels.get(i);
			if (oFF.XString.startsWith(dimension.getName(), leafQueryModel.getDatasetEpmObject().getCubeId()))
			{
				var unAliasedDimensionName = oFF.XString.substring(dimension.getName(), oFF.XString.size(leafQueryModel.getDatasetEpmObject().getCubeId()) + 1, oFF.XString.size(dimension.getName()));
	...
	activateBlendedDimensionHierarchies:function(blendedQueryManager)
	{
			var blendedQueryModel = blendedQueryManager.getQueryModel();
		...
		var leafQueryModels = oFF.BlendingUtils.getLeafQueryModels(blendedQueryManager);
		for (var i = 0; i < dimensionsInDrill.size(); i++)
		{
			...
			if (dimensionInDrill.supportsHierarchy())
			{
				...
				if (dimensionInDrill.getDimensionType() !== oFF.DimensionType.ACCOUNT)
				{
					var leafDimension = oFF.BlendingUtils.findLeafDimensionFromBlendedDimension(leafQueryModels, dimensionInDrill);
					if (oFF.notNull(leafDimension))
					{
						if (leafDimension.isHierarchyActive())
						{
							...
							dimensionInDrill.setInitialDrillLevel(leafDimension.getInitialDrillLevel());
						...
						var hierarchyDisplayKeyField = dimensionInDrill.getHierarchyDisplayKeyField();
						if (oFF.notNull(hierarchyDisplayKeyField) && dimensionInDrill.getResultSetFields().getFieldByName(hierarchyDisplayKeyField.getName()) === null)
						{
							dimensionInDrill.getResultSetFields().add(hierarchyDisplayKeyField);
						}
					}
				}
			}
	...
	checkDimensionLinkPart:function(dimensionLinkPart, datasetId, dimensionName)
	{
	...
	compareLinkedDatasetId:function(dimensionLink, datasetId, dimensionName, linkedDatasetId)
	{
			var firstPart = dimensionLink.getFirstPart();
		var secondPart = dimensionLink.getSecondPart();
		var isFirstPartMatched = oFF.BlendingUtils.checkDimensionLinkPart(firstPart, datasetId, dimensionName) && oFF.XString.isEqual(linkedDatasetId, secondPart.getQueryModel().getDatasetId());
		var isSecondPartMatched = oFF.BlendingUtils.checkDimensionLinkPart(secondPart, datasetId, dimensionName) && oFF.XString.isEqual(linkedDatasetId, firstPart.getQueryModel().getDatasetId());
		return isFirstPartMatched || isSecondPartMatched;
	},
	isDimensionFromLinkedDimension:function(blendableQueryManager, dimensionName, datasetId)
	{
		...
		while (dimensionLinksIterator.hasNext())
		{
			...
			var primaryDatasetId = blendableQueryManager.getPrimaryQueryManager().getQueryModel().getDatasetId();
			if (oFF.BlendingUtils.checkDimensionLinkPart(dimensionLink.getFirstPart(), datasetId, dimensionName) || oFF.BlendingUtils.checkDimensionLinkPart(dimensionLink.getSecondPart(), datasetId, dimensionName))
			{
				if (!oFF.XString.isEqual(datasetId, primaryDatasetId))
				{
					if (oFF.BlendingUtils.compareLinkedDatasetId(dimensionLink, datasetId, dimensionName, primaryDatasetId))
					{
				...
				{
					var secondaryQueryManagers = blendableQueryManager.getSecondaryQueryManagers();
					for (var i = 0; i < secondaryQueryManagers.size(); i++)
					{
						var secondaryDatasetId = secondaryQueryManagers.get(i).getQueryModel().getDatasetId();
						if (oFF.BlendingUtils.compareLinkedDatasetId(dimensionLink, datasetId, dimensionName, secondaryDatasetId))
						{
							isFromLinkedDimension = true;
						}
					}
	...
	getBlendedMeasureMemberFromLeafMeasureMember:function(measureMemberName, isPrimaryMeasure, datasetId, blendableQueryManager, blendedQueryModel)
	{
		...
		var leafQueryModel = leafQueryManager.getQueryModel();
		var aliasPrefix = oFF.BlendingUtils.getAliasPrefixFromQueryModel(blendedQueryModel, leafQueryModel);
		var dimension = isPrimaryMeasure ? blendedQueryModel.getPrimaryCalculationDimension() : blendedQueryModel.getSecondaryCalculationDimension();
		var structureMember = dimension.getStructureMember(oFF.XStringUtils.concatenate2(aliasPrefix, measureMemberName));
		if (oFF.isNull(structureMember))
		{
			var leafDimension = isPrimaryMeasure ? leafQueryModel.getPrimaryCalculationDimension() : leafQueryModel.getSecondaryCalculationDimension();
			var leafStructureMember = leafDimension.getStructureMember(measureMemberName);
			if (oFF.notNull(leafStructureMember) && leafStructureMember.getAliasName() !== null)
			{
				var placeholderId = leafDimension.getRuntimePlaceholderIdByAlias(leafStructureMember.getAliasName());
...
{
	var usedCustomHierarchiesMap = oFF.XLinkedHashMapByString.create();
	var usedHierarchyNames = this.getHierarchyNamesUsedInQueryModel(queryModel);
	var iterator = usedHierarchyNames.getIterator();
	while (iterator.hasNext())
	{
		var customHierarchyDefinition = this.getCustomHierarchyDefinitionByName(iterator.next());
		if (oFF.notNull(customHierarchyDefinition))
		{
			usedCustomHierarchiesMap.put(customHierarchyDefinition.getName(), customHierarchyDefinition);
		}
	}
	return usedCustomHierarchiesMap;
};
oFF.CustomHierarchyRepository.prototype.getHierarchyNamesUsedInQueryModel = function(queryModel)
{
	...
	{
		this.addHierarchyNamesUsedByMembers(queryModel.getMeasureDimension().getExtendedStructureMembers(), usedHierarchyNames);
	}
	usedHierarchyNames.addAll(queryModel.getFilter().getEffectiveFilter().getUsedHierarchyNames());
	return usedHierarchyNames;
};
oFF.CustomHierarchyRepository.prototype.addHierarchyNamesUsedByDimensions = function(dimensions, usedHierarchyNames)
{
	if (oFF.notNull(dimensions))
	{
		for (var i = 0; i < dimensions.size(); i++)
		{
			...
			var hierarchyName = dim.getHierarchyName();
			if (dim.isHierarchyActive() && oFF.XStringUtils.isNotNullAndNotEmpty(hierarchyName))
			{
				usedHierarchyNames.add(hierarchyName);
			}
		}
	}
};
oFF.CustomHierarchyRepository.prototype.addHierarchyNamesUsedByMembers = function(members, usedHierarchyNames)
{
	if (oFF.notNull(members))
	{
		for (var i = 0; i < members.size(); i++)
		{
			...
			{
				usedHierarchyNames.addAll(member.getFilter().getUsedHierarchyNames());
			}
		}
	}
};
...
oFF.BlendableQMPersistedInAExportAction.create = function(blendableQueryManager)
{
	...
	runtimeBlendedQueryManagerAction.m_blendingProcess = oFF.BlendingProcess.create(blendableQueryManager, blendableQueryManager.getBlendingProcessConfigs(), null, null);
...
oFF.BlendableQMPersistedInAExportAction.prototype.onQueryManagerCreated = function(extResult, queryManager, customIdentifier)
{
	...
	if (!extResult.hasErrors())
	{
		...
		for (var i = 0; i < blendingSources.size(); i++)
		{
			...
			var remoteSystemDescription = null;
			...
			{
				var iterator = preQueries.getIterator();
				while (iterator.hasNext())
				{
					var preQueryPair = iterator.next();
					var preQueryModel = preQueryPair.getObject();
					remoteSystemDescription = preQueryModel.getSystemDescription();
					if (oFF.notNull(remoteSystemDescription) && !oFF.XString.isEqual(remoteSystemDescription.getName(), localSystemDescription.getName()))
					{
...
oFF.BlendableQMPersistedInAExportAction.prototype.convertToBrowserSerializedDataQuery = function(originalQuery)
{
	...
	inaDataSource.remove("System");
	var batchQuery = originalQuery.getStructureByKey(oFF.ConnectionConstants.INA_BATCH);
...
oFF.OlapEnvSupportedFormulaAction.prototype.processSyncAction = function(syncType, listener, customIdentifier)
{
	...
	if (systemType.isTypeOf(oFF.SystemType.ABAP) && systemConnect.getServerMetadata().supportsAnalyticCapability(oFF.InACapabilities.C167_FORMULA_OPERATORS_CATALOG) || systemType.isTypeOf(oFF.SystemType.HANA))
	{
	...
	if (systemConnect.getServerMetadata().supportsAnalyticCapability(oFF.InACapabilities.C153_VARIANCE_OPERATOR))
	{
...
oFF.OlapEnvSystem.prototype.processCapabilities = function(syncType, listener, identifier)
{
	...
	{
		var capabilityAction = oFF.OlapEnvCapabilityAction.create(this.getApplication().getSystemConnect(this.m_systemName));
...
{
	var systemConnect = this.getApplication().getSystemConnect(this.m_systemName);
	var systemType = systemConnect.getSystemDescription().getSystemType();
...
oFF.QueryManagerProcessingSequence.prototype.processSynchronization = function(syncType)
{
	...
	if (oFF.QueryManagerProcessingSequence.supportsCustomHierarchies(queryModel))
	{
		...
		var usedCustomHierarchiesMap = customHierarchyRepo.getCustomHierarchiesUsedInQueryModel(queryModel);
		this.processCustomHierarchySubmits(usedCustomHierarchiesMap.getValuesAsReadOnlyList(), this.m_enforceCustomHierarchyRequests);
	}
...
oFF.BlendingProcess.create = function(blendableQueryManager, configs, runtimeBlendedQueryManager, lastModCounterState)
{
...
oFF.BlendingProcess.prototype.validateCachedRuntimeBlendedQueryManager = function(blendableQueryManager, runtimeBlendedQueryManager, previousMeasureFilterModCounters)
{
	...
	while (iterator.hasNext())
	{
		...
		var previousModCounter = oFF.notNull(previousMeasureFilterModCounters) ? previousMeasureFilterModCounters.getByKey(queryModel.getDatasetId()) : null;
...
oFF.BlendingProcess.prototype.cleanupTransientState = function(queryManager)
{
	...
	else
	{
		...
		if (oFF.notNull(transientMembers))
		{
			...
			{
				var transientMember = transientMembersIter.next();
				if (transientMember.getTagging().containsKey("isTransient") || oFF.XString.isEqual(transientMember.getAliasOrMemberName(), "DummyCalculation"))
				{
...
oFF.BlendingProcess.prototype.onQueryManagerCreated = function(extResult, queryManager, customIdentifier)
{
	...
	else
	{
		...
		oFF.BlendingUtils.activateBlendedDimensionHierarchies(queryManager);
...
oFF.BlendingProcess.prototype.applyMeasureSort = function(blendableMeasureSort)
{
	...
	if (blendableMeasureSort.getPrimaryMeasureName() !== null)
	{
		...
		if (blendableMeasureSort.getPrimaryDatasetId() !== null)
		{
	...
	if (blendableMeasureSort.getSecondaryMeasureName() !== null)
	{
		...
		if (blendableMeasureSort.getSecondaryDatasetId() !== null)
		{
...
oFF.BlendingProcess.prototype.prepareQueryExecution = function()
{
	if (oFF.notNull(this.m_blendedQueryManager))
	{
		...
		oFF.BlendingUtils.activateBlendedDimensionHierarchies(this.m_blendedQueryManager);
	...
	if (oFF.isNull(this.m_cursorResultSetInSync))
	{
		...
		if (syncState === oFF.SyncState.OUT_OF_SYNC)
		{
			...
			if (oFF.isNull(this.m_blendingProcess) || this.m_blendingProcess.isSyncCanceled())
			{
				...
				this.m_blendingProcess = oFF.BlendingProcess.create(blendableQueryManager, configs, this.m_runtimeQueryManager, this.m_lastModCounterState);
			}
...
oFF.BlendableResultSetContainer.prototype.createBlendedQueryManager = function(syncType, listener, customIdentifier)
{
	...
	{
		this.m_blendingProcess = oFF.XObjectExt.release(this.m_blendingProcess);
		this.m_blendingProcess = oFF.BlendingProcess.create(blendableQueryManager, blendableQueryManager.getBlendingProcessConfigs(), this.m_runtimeQueryManager, this.m_lastModCounterState);
	}
...
oFF.OlapFilterManager.prototype.externalizeNonVariableFilter = function(queryManager)
{
	...
	if (oFF.notNull(cartesianProduct))
	{
		...
		while (dimensionNamesIterator.hasNext())
		{
			var dimensionName = dimensionNamesIterator.next();
			var list = cartesianProduct.getCartesianListByDimensionName(dimensionName);
			if (oFF.notNull(list) && !variableDimensionNames.contains(dimensionName))
			{
...
oFF.QGeoManager.prototype._loadChoroplethHierarchies = function(syncType, listener, systemName, liveSynonymSupportEnabled, localSynonymSupportEnabled, customIdentifier)
{
	...
	if (oFF.isNull(hierarchiesResult))
	{
		...
		{
			var systemConnect = this.getApplication().getSystemConnect(theSystemName);
...
oFF.QGeoManager.prototype.getChoroplethExtendedDimension = function(systemName, hierarchyId, levelId, geoDimensionName, parentQueryManager)
{
	...
	{
		var systemConnect = this.getApplication().getSystemConnect(systemName);
		var serverMetadata = systemConnect.getServerMetadata();
